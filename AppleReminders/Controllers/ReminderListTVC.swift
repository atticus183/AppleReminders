...
class ReminderListTVC: UIViewController {
    ...
    @objc func navBarBtnTapped() {
        let actionSheet = UIAlertController(title: "Choose Account", message: nil, preferredStyle: .actionSheet)
        
        //Name & Appearance -->  Brings up CreateListVC with passed values
        let nameAndAppearance = UIAlertAction(title: "Name & Appearance", style: .default) { [weak self] _ in
        ...
        let viewParticipants = UIAlertAction(title: "View Participants", style: .default) { [weak self] _ in
            self?.alert(message: "Feature not implemented.", title: "Feature Unavailable")
        }
        
        //Delete List -->  Ask confirmation to delete the list
        let deleteList = UIAlertAction(title: "Delete List", style: .default) { [weak self, weak passedReminderList] _ in
            //Present confirmation
            let listName = passedReminderList!.name
            let ac = UIAlertController(title: #"Delete "\#(listName)?"#, message: "If you delete this shared list, other people will no longer have access to it and it will be deleted from all of their devices.", preferredStyle: .alert)
        ...
        let selectReminders = UIAlertAction(title: "Select Reminders", style: .default) { [weak self] _ in
        ...
        let showHideText = reminderDatasource.showCompleted ? "Hide Completed" : "Show Completed"
...
//        print("IndexPath.section = \(indexPath.section), IndexPath.row = \(indexPath.row)")
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let flag = UIContextualAction(style: .normal, title: "Flag") { [weak self] (_, _, _)  in
            guard let self = self else { return }
